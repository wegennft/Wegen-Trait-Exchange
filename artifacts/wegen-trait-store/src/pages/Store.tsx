...
import { Loader2, Coins, TrendingUp, Users, Package } from "lucide-react";
...
export function Store() {
  ...
  const { data: traitsData, isLoading: isLoadingTraits } = useListTraits(
    ...
    { query: { queryKey: getListTraitsQueryKey({ category: selectedCategory }) } }
  );
  ...
  const purchaseTrait = usePurchaseTrait({
    mutation: {
      onSuccess: () => {
        ...
        setTraitToBuy(null);
        queryClient.invalidateQueries({ queryKey: getListTraitsQueryKey({ category: selectedCategory }) });
      },
      onError: (error) => {
        toast({
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">
            Acquire exclusive artifacts for your Wegen NFTs. Select categories below to filter the collection.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card/30 border-border/50">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 rounded-full bg-primary/20 text-primary"><Package className="w-5 h-5" /></div>
            <div>
        ...
        <Card className="bg-card/30 border-border/50">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 rounded-full bg-accent/20 text-accent"><Coins className="w-5 h-5" /></div>
            <div>
        ...
        <Card className="bg-card/30 border-border/50">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 rounded-full bg-blue-500/20 text-blue-500"><Users className="w-5 h-5" /></div>
            <div>
        ...
        <Card className="bg-card/30 border-border/50">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 rounded-full bg-green-500/20 text-green-500"><TrendingUp className="w-5 h-5" /></div>
            <div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          className={!selectedCategory ? "bg-primary text-white shadow-[0_0_10px_rgba(157,0,255,0.3)]" : ""}
        >
          All Traits
        </Button>
        {isLoadingCategories ? (
          ...
          categoriesData?.categories?.map((cat) => (
            <Button
              ...
              className={`capitalize ${selectedCategory === cat ? "bg-primary text-white shadow-[0_0_10px_rgba(157,0,255,0.3)]" : ""}`}
            >
              {cat}
            </Button>
          ))
      ...
      {isLoadingTraits ? (
        ...
        <div className="text-center py-20 border border-dashed border-border/50 rounded-xl bg-card/30">
          ...
          <p className="text-muted-foreground">
            {selectedCategory ? `No traits available in the ${selectedCategory} category.` : "The store is currently empty."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            >
              <div className="relative aspect-square overflow-hidden bg-secondary flex items-center justify-center p-6">
                ...
                <div className="absolute top-3 right-3 flex flex-col gap-2">
                  <Badge variant="outline" className={`uppercase tracking-wider text-[10px] font-bold px-2 py-1 ${getRarityColor(trait.rarity)}`}>
                    {trait.rarity}
                  </Badge>
                </div>
            ...
            <div className="py-4">
              <div className="flex items-center gap-4 p-4 rounded-lg bg-secondary/50 border border-border/50 mb-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground capitalize">{traitToBuy.category} &bull; {traitToBuy.rarity}</div>
                </div>
