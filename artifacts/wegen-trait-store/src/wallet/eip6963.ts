...
function onAnnounce(event: Event) {
  ...
  if (detail?.info?.rdns && detail?.provider?.request) {
...
export function requestEip6963Providers(timeoutMs = 600): Promise<Map<string, Eip6963ProviderDetail>> {
...
export function getEip6963Provider(walletId: WalletId): Eip1193Provider | null {
  ...
  for (const rdns of rdnsList) {
    ...
    if (detail?.provider?.request) return detail.provider;
  }
  return null;
}
